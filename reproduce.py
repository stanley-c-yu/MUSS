...
def reproduce(*, force_fresh_data=True, debug=False, parallel=False, profiling=False, run_ts='new'):
    ...
    dataset_path = prepare_validation_set(data_folderpath, output_folder=output_folder, debug=debug, scheduler=scheduler, profiling=profiling, force=force_fresh_data)
    logging.info('Validation sets are saved to {}'.format(dataset_path))
    logging.info('Running validation experiments...')
    prediction_path = run_validation_experiments(data_folderpath, output_folder=output_folder, debug=debug, scheduler=scheduler, profiling=profiling, force=force_fresh_data)
...
if __name__ == "__main__":
    run(reproduce)
    # reproduce(force_fresh_data=False, debug=True, parallel=False, profiling=False, run_ts='2019-06-21-16-21-12')