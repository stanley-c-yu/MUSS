...
def class_mapping(in_annotation):
    ...
    # TODO: filter those with less than "inteval" seconds

    labels = in_annotation.iloc[:,3].unique()
    mapped_list = []
    for label in labels:
        if __skip_invalid(label):
            continue
        ...
        lab_freeliving = __get_lab_freeliving(label, activity)
        ...
        mapped_list.append(pd.Series([label, posture, four_class, activity_group,
                                      ambience, lab_freeliving, activity,hand_gesture],
                                     index=['label','posture','four_classes',
                                            'activity_group','indoor_outdoor','lab_freeliving',
                                            'activity', 'hand_gesture']))
    return pd.DataFrame(mapped_list)

def __skip_invalid(label):
    if any(re.findall(word, label) for word in ['sit', 'reclin']) and any(re.findall(word, label) for word in ['sweep', 'laundry', 'vending machine', '300 kpm', 'walk', 'run', 'treadmill', 'mph', 'bik', 'stair', 'frisbee', 'escalator', 'elevator', 'stand']):
        return True
    if any(re.findall(word, label) for word in ['bik', '300 kpm']) and any(re.findall(word, label) for word in ['sweep', 'laundry', 'vending machine', 'walk', 'run', 'treadmill', 'mph', 'game', 'writing', 'web browsing', 'stair', 'frisbee', 'escalator', 'elevator', 'arms on desk', 'stand']):
        return True
    if any(re.findall(word, label) for word in ['walk', 'run', 'treadmill']) and any(re.findall(word, label) for word in ['vending machine', '300 kpm', 'game', 'writing', 'stand']):
        return True
    if any(re.findall(word, label) for word in ['300 kpm', 'treadmill', 'mph', 'game', 'arms on desk', 'writing']) and any(re.findall(word, label) for word in ['outdoor', 'vending machine', 'stop light']):
        return True
...
def __get_posture(label, activity):
    ...
    sit_keywords = ['sit','bik','reclin','eat', '300 kpm']
    if any(re.findall(word, label) for word in sit_keywords):
        return 'sitting'

    upright_keywords = ['stand', 'run', 'jump', 'walk', 'frisbee', 'escalator', 'shelf'
                        ,'elevator', 'climb.*stair','stair', 'treadmill','laundry','stop light',
...
def __get_lab_freeliving(label, activity):
    lab_keywords = ['in\s*door', 'computer', r'brush\s*teeth'
                       ,'sleep','cook','bed', 'typ','300 kpm/min'
                       ,'treadmill', 'arm.*on desk', 'stationary biking', 'lying','sweep','game','writing','web browsing','frisbee', 'supported',
                       'eat','pack','gam', 'jumping jacks', 'sitting still', 'sitting-writing', 'reclining', 'shelf', 'sitting-telling story', 'laundry', '300 kpm/min-biking']
    if any((re.findall(word, label) or re.findall(word, activity)) for word in lab_keywords):
        return 'lab'

    freeliving_keywords = ['out\s*door', 'signal\s*light', 'city','stop.*light', 'elevator', 'mbta', 'escalator', 'shop', 'stairs', 'telling story-walking', 'vending', 'bag-walking', 'texting-walking', 'self-paced walking']
    if any((re.findall(word, label) or re.findall(word, activity)) for word in freeliving_keywords):
        return 'freeliving'


    print('unknow lab or freeliving: ', label, activity)
    return 'unknown'
...
def __get_activity(label):
    ...
    activities_verbs = collections.OrderedDict({'sitting':['sit'],
                  ...
                  'stationary biking':['bik.*stationary',r'bik.*in\s*door|in\s*door.*bik', '300.*kpm.*bik'],
                  'frisbee':['frisbee'],
    ...
    if 'walk' in label and '300 kpm' in label:
        activity = ''
