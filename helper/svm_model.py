...
def svm_model(input_matrix, input_classes, C=16, kernel='rbf', gamma=0.25, tol=0.0000001):
    ...
    classifier = svm.SVC(C=C, kernel=kernel, gamma=gamma,
                         tol=tol, class_weight="balanced", verbose=False)
...
def loso_validation(input_matrix, input_classes, groups, **model_kwargs):
    ...
    for train_indices, test_indices in loso.split(input_matrix, input_classes, groups=groups):
        ...
        model, scaler = svm_model(train_set, train_labels, **model_kwargs)
