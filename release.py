...
def create_dist(version, *, dataset='./muss_data', sample_result='./muss_data/DerivedCrossParticipants/product_run', message=None):
    ...
    if dataset is None:
        ...
        steps.pack_data(dataset, dist_folder, 'muss_data.tar.gz')
        logging.info('Data asset is packed.')
    if sample_result is None:
        logging.warn(
            'No sample result is provided, skipping packing result asset')
    else:
        logging.info('Packing result assests...')
        steps.pack_data(sample_result, dist_folder,
                        'sample_reproduction_results.tar.gz')
