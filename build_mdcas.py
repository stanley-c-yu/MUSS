...
def build_mdcas(*, force_fresh_data=True, debug=False, parallel=False, profiling=False, run_ts='new'):
    ...
    dataset_path = prepare_validation_set(data_folderpath, output_folder=output_folder, sites='DW', feature_types='MO', include_nonwear=True, debug=debug, scheduler=scheduler, profiling=profiling, force=force_fresh_data, target='MDCAS')
    logging.info('Validation set are saved to {}'.format(dataset_path))
    logging.info('Running validation experiments...')
    prediction_path = run_validation_experiments(data_folderpath, output_folder=output_folder, sites='DW', feature_set='MO', target='MDCAS', include_nonwear=True, debug=debug, scheduler=scheduler, profiling=profiling, force=force_fresh_data)
    logging.info('Prediction results are saved to {}'.format(prediction_path))
    logging.info('Train and save MDCAS model...')
    model_folder = train_and_save_model(data_folderpath, output_folder=output_folder, debug=debug, targets='MDCAS', feature_set='MO', sites='DW')
    logging.info('Model is saved to {}'.format(model_folder))


if __name__ == "__main__":
    # run(reproduce)
    build_mdcas(force_fresh_data=False, debug=False, parallel=True, profiling=False, run_ts='new')